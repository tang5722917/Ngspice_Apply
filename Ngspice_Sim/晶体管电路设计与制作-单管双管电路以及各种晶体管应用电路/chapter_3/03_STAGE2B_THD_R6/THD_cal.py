filename = "03_STAGE2B_THD_R6.CIR"
min_VIN = 10
max_VIN = 400
Change_num = 1
...
'''
sim_point = range(min_VIN,max_VIN)
result_list = list()
result_list_I = list()
result_item = list()
    ...
    for j in range(0,len(sim_point)):
        ...
        for data_line in target.readlines():
            if "fourier" in data_line :
                print("fourier 1000 V(9) > " + filename+"_"+str(i)+"_"+str(j)+".out.TEMP",file=result)
            elif "<Change>" in data_line:
                str_change_data = ' '
                if data_line.split() [1] == str(i+1 ) :
                    for data in data_line.split() [2:]:
                        if (data == "<Value>") :
                            str_change_data = str_change_data + " " + str(sim_point[j]*100)
                        else:
...
for i in range(0,len(sim_point)):
    for j in range(0, Change_num ):
        out_line = out_line + str(sim_point[i]/10.0) + 'k ' + result_list[j][i][0]
