...
if __name__ == '__main__':
    ...
    PARAM_Unit = "k"
    # Netlist 文件名
    filename = "6Tr_Emmitter_follower"
    # THD 测试RL电阻范围
    min_VIN = 0.4
    max_VIN = 10
    # 电路变量个数
    Change_num = 2
    Change_list = ["1000","20000"]
    # 观测节点 
    Node = 'V(OUT)'
    ...
    for i in range(0,Change_num):
        for j in range(0,len(sim_point)):
            ...
            print("alterparam "+ PARAM_name + " = "+ str(format(sim_point[j], '0.2f')) + PARAM_Unit ,file=result)
            print("reset \nrun" ,file=result)
    ...
    for i in range(0,len(sim_point)):
        for j in range(0, Change_num ):
            out_line = out_line + str(sim_point[i]) + ' ' + result_list[j][i][0] + ' '
